...
const { safeSendEmail } = require("./emailService");
    ...
    try {
      ...
      const isBirthday = 
        ...
        if (result.ok) {
          ...
          failed++;
          console.warn(`Failed to send birthday email to ${customer.email} - ${result.errorType}`);
        }
  ...
  try {
    ...
    for (const customer of customers) {
      try {
        ...
        if (result.ok) {
          sent++;
          console.log(`✅ Test email sent to ${customer.email}`);
        } else {
          failed++;
          console.warn(
            `⚠️ Test email failed for ${customer.email} — ${result.errorType}`
          );
        }
    ...
    console.error("❌ Test email job crashed:", error);
    return { sent: 0, failed: 0 };
  }
};
